...
function activateTab(nextTab) {
	grabTabSettings(nextTab.windowId, nextTab, function(tabSetting){
		// console.log(tabSetting.current_tab_round); console.log(tabSetting.seconds); console.log(tabSetting.reload_after_rounds); console.log(typeof tabSetting.current_tab_round); console.log(typeof tabSetting.reload_after_rounds);
		if(tabSetting.reload &&  tabSetting.current_tab_round == tabSetting.reload_after_rounds && !include(settings.noRefreshList, nextTab.url) && nextTab.url.substring(0,19) != "chrome://extensions"){
			tabSetting.current_tab_round = 0;
			...
			tabSetting.current_tab_round = tabSetting.current_tab_round + 1;
...
function assignBaseSettings(tabs, callback) {
	for(var i = 0;i<tabs.length;i++){
		tabs[i].current_tab_round = 0;
		tabs[i].reload = (tabs[i].reload || settings.reload);
		tabs[i].seconds = (tabs[i].seconds || settings.seconds);
		tabs[i].reload_after_rounds = (tabs[i].reload_after_rounds || settings.reload_after_rounds);
	};
	callback();
}
	...
	for(var y=0;y<tabs.length;y++){
		for(var i=0;i<advSettings.length;i++){
			if(advSettings[i].url == tabs[y].url) {
				...
				tabs[y].reload_after_rounds = advSettings[i].reload_after_rounds;
			}
		}
	}
	callback();
...
function checkForAndMigrateOldSettings(callback){
	if(localStorage["revolverSettings"]) callback();
		...
		var oldSettings = ["seconds", "autostart", "inactive", "noRefreshList", "reload", "reload_after_rounds"],
			tempSettings = {};
...
function createBaseSettingsIfTheyDontExist(){
	if(!localStorage["revolverSettings"]){
		...
		settings.reload_after_rounds = 10;
