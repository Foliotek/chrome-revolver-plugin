...
function saveBaseOptions(callback) {
  ...
  appSettings.reload_after_rounds = document.getElementById("reload_after_rounds").value;
...
function restoreOptions() {
  ...
  document.getElementById("reload_after_rounds").value = (appSettings.reload_after_rounds || 10);
  ...
  for (var i = 0, checkboxes = 0; i < advancedDivs.length; i++) {
    if (advancedDivs[i].getElementsByClassName("enable")[0].checked == true) {
      ...
      advUrlObjectArray.push({
        ...
        "reload_after_rounds": divInputTags[4].value,
...
function generateAdvancedSettingsHtml(tab, saved) {
  var advancedSettings = document.getElementsByClassName("adv-settings")[0],
    ...
    secondsChunk = '<p><label for="seconds">Change tab after </label> <input type="text" name="seconds" value="10" style="width:30px;"> <label class="label-close">Seconds</label>',
    reloadChunk = '<label class="inline" for="reload">Reload</label> <input type="checkbox" name="reload"> <label class="inline label-close" for="reload_after_rounds">After</label> <input type="text" name="reload_after_rounds" value="10" style="width:30px;" /><label class="inline label-close">Rounds</label></p></div>';
  if (saved) {
    ...
    if (tab.reload) {
      reloadChunk = '<label class="inline" for="reload">Reload</label> <input type="checkbox" name="reload" checked><label class="inline label-close" for="reload_after_rounds">After</label> <input type="text" name="reload_after_rounds" value="' + tab.reload_after_rounds + '" style="width:30px;" /><label class="inline label-close">Rounds</label></p></div>';
    }
  }
